...
$$q=\text{clip}\big(\text{round}(x/s) + z,\ q_{\min},\ q_{\max}\big),\qquad \hat{x}=s\,\big(q-z\big).$$
...
$$\alpha^\star=\arg\min_{\alpha}\ \text{KL}\!\left(P\;\|\;Q_\alpha\right)=\arg\min_{\alpha}\ \sum_i P(i)\log\frac{P(i)}{Q_\alpha(i)}.$$
...
Freeze a FP32 *teacher* network $F^{\text{fp}}$. Quantize layers progressively. For layer $\ell$, learn that layer's quantization parameters (and sometimes its rounding) by matching its FP32 *outputs* given real activations.

**Objective (layer-wise reconstruction):**  
Let $h_{\ell-1}$ be the (already‑quantized) input activations into layer $\ell$. Define

$$y_\ell^{\text{fp}} = F_\ell^{\text{fp}}(h_{\ell-1}),\qquad y_\ell^{q} = F_\ell^{q}(h_{\ell-1};\,\theta_\ell^q),$$

where $\theta_\ell^q$ are the scales/zero‑points (and possibly rounding parameters) for layer $\ell$. Solve

$$\theta_\ell^{q\star}=\arg\min_{\theta_\ell^q}\ \mathbb{E}_{h_{\ell-1}\sim\mathcal{D}}\big[\|y_\ell^{\text{fp}}-y_\ell^{q}\|_2^2\big] + \lambda\,\Omega(\theta_\ell^q).$$
...
$$\min_{\theta_\ell^q}\ (y_\ell^{q}-y_\ell^{\text{fp}})^\top H_\ell \,(y_\ell^{q}-y_\ell^{\text{fp}}),$$
...
$$W' = \frac{\gamma}{\sqrt{\sigma^2+\epsilon}}\,W,\qquad b' = \frac{\gamma}{\sqrt{\sigma^2+\epsilon}}\,(b-\mu)+\beta.$$
...
$$(a^\star,c^\star)=\arg\min_{a,c}\ \mathbb{E}\big[\|y^{\text{fp}} - (a\odot \hat{y} + c)\|_2^2\big],$$
...
$$\min_{\{b_\ell\}} \sum_\ell \text{Err}_\ell(b_\ell)\quad \text{s.t.}\ \sum_\ell \text{Cost}_\ell(b_\ell)\le B,\ \ b_\ell\in\{4,8,16,32\}.$$
...
$$\text{MSE}(\alpha) = \mathbb{E}_{x\sim\mathcal{D}}\!\left[ \big(x - \hat{x}_\alpha\big)^2 \right],\quad \alpha^\star = \arg\min_{\alpha} \text{MSE}(\alpha).$$
...
$$\text{KL}\big(P\ \|\ Q_\alpha\big)=\sum_{i} P(i)\log \frac{P(i)}{Q_\alpha(i)},\quad \alpha^\star=\arg\min_{\alpha}\ \text{KL}\big(P\ \|\ Q_\alpha\big).$$
...
4. **Layer‑wise reconstruction (AdaQuant/AdaRound):** greedily minimize $\|y_\ell^{\text{fp}}-y_\ell^q\|^2$ on calibration batches.  
...
$$\underbrace{\mathbb{E}\big[(x-\text{clip}(x,\!-\alpha,\alpha))^2\big]}_{\text{clipping error}} \;+\; \underbrace{\frac{s^2}{12}}_{\text{rounding error (high‑resolution approx.)}},\quad s=\alpha/Q,\ Q=2^{b-1}-1.$$
